...
export default class Step extends Component {
  ...
  render() {
    const { title, icon, index, active, completed, first, isLast, href, onClick, completeIcon } = this.props;
    ...
    const comIcon = completeIcon?<img style={ styles.imageIcon } src={completeIcon} alt={index + 1} /> : index + 1;

    return (
      <div style={ styles.step }>
        <div style={ circleStyle }>
        {/*{completed ? (
          <a href={href} onClick={onClick} style={ styles.index }>{ comIcon }</a>
        ) : (
        ...
        {completed? (<a href={href} onClick={onClick} style={ styles.index }>{ comIcon }</a>):active?(<a href={href} onClick={onClick} style={ styles.index }>{ stepContent }</a>):(<span style={ styles.index }>{ stepContent }</span>)}
        </div>
        {active || completed ? (
